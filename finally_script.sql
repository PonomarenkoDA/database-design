...
INSERT INTO workers (name_worker, exp_worker, phone_worker, wages, service_id)
	SELECT DISTINCT
		w_name,
		w_exp,
		w_phone,
...
INSERT INTO cars (car, vin, car_number, color, client_id)
	SELECT DISTINCT
		...
		car_number,
		color,
		(SELECT client_id FROM clients WHERE client_name = tb.name)
	FROM cleaned_repair_service AS tb
...
INSERT INTO orders (date, payment, mileage, car_id, worker_id, card_id)
	SELECT
		...
		(SELECT worker_id FROM workers WHERE name_worker = tb.w_name),
...
CREATE INDEX name_worker_idx ON workers (name_worker);
CREATE INDEX name_client_idx ON clients (client_name);
